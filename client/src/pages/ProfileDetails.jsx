...
export default function ProfileDetails() {
    ...
    const getFullImageUrl = (path) => {
         // This ensures the URL starts with the full protocol and host (like your IndexPage)
        return `http://localhost:4000/${path.startsWith('/') ? path.substring(1) : path}`;
    };

    // Determine what to render for the photo area
    const renderProfilePhoto = () => {
        // Case 1: No photo path in DB OR image failed to load
        if (!photoPath || imageLoadError) {
            return (
                <FaUserCircle 
                    className="w-32 h-32 md:w-40 md:h-40 text-gray-500 rounded-full border-4 border-yellow-400 flex-shrink-0 shadow-lg mb-4 sm:mb-0 sm:mr-8" 
                    title="No Profile Picture"
                />
            );
        }

        // Case 2: Photo path exists, attempt to load the image
        return (
            <img 
                ...
                onError={() => setImageLoadError(true)} // Use the state hook for clean error fallback
            />
        );
    }
                    ...
                    <div className="bg-gray-800 p-6 rounded-lg shadow-inner border border-gray-700">
                        ...
                        {user.preferences && typeof user.preferences === 'object' && Object.keys(user.preferences).length > 0 && (
                            <p className="text-gray-300 mt-2">
                                Preferences: <span className="font-mono text-sm text-yellow-400">{JSON.stringify(user.preferences)}</span>
                            </p>
                        )}
                    </div>
